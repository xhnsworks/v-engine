...
_INLINE_ void tree_node_Init(struct _tree* _tree, struct tree_node* _node, var _key, var _data)
{
    ...
    {
        var value;
        value.str_var = EString_new((char*)_data.str_var);
        _node->value = value;
    }
...
Iterator Tree_force_find(Tree _tree, var _key)
{
    ...
    if (unlikely(!_tree->root))
    {
        _tree->root = _tree->head = _tree->tail = _tree->alloc_proc(sizeof(struct tree_node));
    ...
    node = _tree->alloc_proc(sizeof(struct tree_node));
...
void Tree_set_data_compare_proc(Tree _tree, DATA_COMPARE _proc)
{
    EAssert(_tree->key_type == Vptr, "Tree_set_data_compare_proc fail");
    _tree->data_compare_proc = _proc;
}
