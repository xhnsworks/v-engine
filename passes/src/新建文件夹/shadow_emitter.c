...
PxlSdrBuf create_shadow2d_pixel_shader_buffer(VertexDecl _dec, int num_shadow_emitters)
{
    ...
    for (int i = 0; i < num_shadow_emitters; i++)
    {
        char mbuf0[256];
        char mbuf1[256];
        snprintf(mbuf0, 255, "UnifLightWorldMatrix%d", i);
        snprintf(mbuf1, 255, "UnifLightProjectionMatrix%d", i);
        ShaderBuffer_add_uniform(sb, Matrix4x4_Param, mbuf0, 1, LightWorldMatrix0 + i);
        ShaderBuffer_add_uniform(sb, Matrix4x4_Param, mbuf1, 1, LightProjectionMatrix0 + i);
    }
...
Pass create_shadow_pass(VertexDecl _dec, int num_shadow_emitters, int idx_emitter)
{
    char mbuf[STRING_BUFFER_SIZE];

    VtxSdrBuf vsb = create_std_vertex_shader_buffer(_dec, true, true);
    PxlSdrBuf psb = create_shadow2d_pixel_shader_buffer(_dec, num_shadow_emitters);
    ...
    ShaderObject shadow_test_result = IPxlSdrBuf.new_object(psb, Bool_Obj, "ShadowTestResult", 1);
    ShaderObject depth_map = ShaderBuffer_find_object(sb, "UnifDepthMap");
    char mbuf0[256];
    char mbuf1[256];
    snprintf(mbuf0, 255, "UnifLightWorldMatrix%d", idx_emitter);
    snprintf(mbuf1, 255, "UnifLightProjectionMatrix%d", idx_emitter);
    ShaderObject light_world_matrix = ShaderBuffer_find_object(sb, mbuf0);
    ShaderObject light_proj_matrix = ShaderBuffer_find_object(sb, mbuf1);
