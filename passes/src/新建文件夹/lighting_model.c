...
ShaderNode create_direction_lighting_node()
{
    ...
    snprintf(mbuf, STRING_BUFFER_SIZE - 1,
             ...
             "    d = dot(r, eye_dir);\n"
             "    d = clamp(d, 0.0, 1.0);\n"
             "    vec3 s = pow(d, 4.0) * LightColor.rgb;\n"
             /// output
             "    OutDiffuse = InDiffuse + c;\n"
             "    OutSpecular = InSpecular + s;\n"
             "}\n");
...
{
    ShaderObject nor = IPxlSdrBuf.new_object(_psb, Float4_Obj, "Normal", 1);
    ShaderObject depth = IPxlSdrBuf.new_object(_psb, Float4_Obj, "Depth", 1);
    ShaderObject tgt_pos = IPxlSdrBuf.new_object(_psb, Float3_Obj, "TargetPosition", 1);
    ShaderObject tgt_nor = IPxlSdrBuf.new_object(_psb, Float3_Obj, "TargetNormal", 1);
    ///ShaderObject tgt_spe = IPxlSdrBuf.new_object(_psb, Float_Obj, "TargetSpecular", 1);
    ShaderObject diffuse = IPxlSdrBuf.new_object(_psb, Float3_Obj, "DiffuseValue", 1);
    ShaderObject specular = IPxlSdrBuf.new_object(_psb, Float3_Obj, "SpecularValue", 1);
    ShaderObject view_light_dir = IPxlSdrBuf.new_object(_psb, Float3_Obj, "ViewLightDir", 1);

    uint _num_shadow_map = _lt_state->num_shadow_emitters;
    EAssert(_num_shadow_map <= 6, "%s", "num shadow map is more than 6");

    ShaderObject shadow_maps[_num_shadow_map];
    ShaderObject light_world_mats[_num_shadow_map];
    ShaderObject light_proj_mats[_num_shadow_map];
    ShaderObject shadow_dirs[_num_shadow_map];
    /// 如果阴影贴图数量为0，表示不使用阴影
    for (uint i = 0; i < _num_shadow_map; i++)
    {
        ...
        /**
        IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, ShadowMap0 + i, shadow_map);
        IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, LightWorldMatrix0 + i, light_world_matrix);
        IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, LightProjectionMatrix0 + i, light_proj_matrix);
        IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, ShadowDirection0 + i, shadow_dir);
        **/
        shadow_maps[i] = IPxlSdrBuf.find_object(_psb, shadow_map);
        light_world_mats[i] = IPxlSdrBuf.find_object(_psb, light_world_matrix);
        light_proj_mats[i] = IPxlSdrBuf.find_object(_psb, light_proj_matrix);
        shadow_dirs[i] = IPxlSdrBuf.find_object(_psb, shadow_dir);
    }

    ShaderObject nor_map = IPxlSdrBuf.find_object(_psb, NORMAL_MAP);
    ShaderObject depth_map = IPxlSdrBuf.find_object(_psb, DEPTH_MAP);
    ShaderObject tex_crd = IPxlSdrBuf.find_object(_psb, TEXCOORD);
    ShaderObject width =  IPxlSdrBuf.find_object(_psb, CAMERA_PLANE_WIDTH);
    ShaderObject height =  IPxlSdrBuf.find_object(_psb, CAMERA_PLANE_HEIGHT);
    ShaderObject near_plane =  IPxlSdrBuf.find_object(_psb, CAMERA_PLANE_NEAR);
    ShaderObject far_plane =  IPxlSdrBuf.find_object(_psb, CAMERA_PLANE_FAR);

    ShaderObject inv_cam_world_mat =  IPxlSdrBuf.find_object(_psb, INVERT_CAMERA_WORLD_MATRIX);
    ShaderObject inv_cam_proj_mat =  IPxlSdrBuf.find_object(_psb, INVERT_CAMERA_PROJECTION_MATRIX);

    ShaderObject cam_world_mat = IPxlSdrBuf.find_object(_psb, CAMERA_WORLD_MATRIX);
    ShaderObject cam_pos = IPxlSdrBuf.find_object(_psb, CAMERA_POSITION);

    ShaderObject light_pos = IPxlSdrBuf.find_object(_psb, LIGHT_POSITION);
    ShaderObject light_dir = IPxlSdrBuf.find_object(_psb, LIGHT_DIRECTION);
    ShaderObject light_col = IPxlSdrBuf.find_object(_psb, LIGHT_COLOR);
    ShaderObject light_ac = IPxlSdrBuf.find_object(_psb, LIGHT_ATTE_COEF);

    if (_has_lighting_map)
    {
        ShaderNode sampler_node = ISdrNdGen.add_reference_node_2(_sng, _cb, MapSampleNode);
        ShaderObject diffuse_map = IPxlSdrBuf.find_object(_psb, DIFFUSE_LIGHTING_MAP);
        ...
        ShaderObject specular_map = IPxlSdrBuf.find_object(_psb, SPECULAR_LIGHTING_MAP);
        ShaderNode_add_input_link(sampler_node, diffuse_map, INVALID_ARRAY_INDEX);
        ShaderNode_add_input_link(sampler_node, tex_crd, INVALID_ARRAY_INDEX);
    ...
    }
    {
        if (_num_shadow_map == 1) {
            ///ShaderNode shadow_test_node = ISdrNdGen.add_reference_node_2(_sng, _cb, ShadowMapTestNode);
/**
...
**/
    ...
    }
    {
        ...
        ShaderNode out_node = ISdrNdGen.add_reference_node_2(_sng, _cb, LightingOutputNode);
...
PxlSdrBuf create_lighting_pixel_shader_buffer2(Renderer _rdr, VertexDecl _dec, uint _num_shadow_map, bool _has_lighting_map)
{
    /**
    ...
    **/
    ...
    IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, CameraPosition, CAMERA_POSITION);

    IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, LightPosition, LIGHT_POSITION);
    IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, LightDirection, LIGHT_DIRECTION);
    IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, LightColor, LIGHT_COLOR);
    IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, LightAtteCoef, LIGHT_ATTE_COEF);
    ...
    if (_has_lighting_map)
    {
        IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, DiffuseLightingSketch, DIFFUSE_LIGHTING_MAP);
        IPxlSdrBuf.add_uniform_from_renderer(ret, _rdr, SpecularLightingSketch, SPECULAR_LIGHTING_MAP);
    }
    /// 如果阴影贴图数量为0，表示不使用阴影
    for (uint i = 0; i < _num_shadow_map; i++)
    {
...
Pass create_lighting_pass_ex3(Renderer _rdr, VertexDecl _dec, LightState _lt_state, lighting_decl* _decls, bool _has_lighting_map)
{
    ...
    PxlSdrBuf psb = create_lighting_pixel_shader_buffer2(_rdr, _dec, _lt_state->num_shadow_emitters, _has_lighting_map);

    ShaderNode vsn = create_lighting_vertex_shader_node(_dec);

    IVtxSdrBuf.add_prototype_node(vsb, vsn);
    IVtxSdrBuf.add_reference_node(vsb, ShaderNode_get_name(vsn));
    ...
    ISdrNdGen.register_shader_node(sng, ShadowMapTestNode, create_shadow2d_test_node);
    ISdrNdGen.attach_all_prototype_nodes(sng, sb);

    ShaderObject mat_id_tex = IPxlSdrBuf.find_object(psb, "MaterialIDMap");
    ShaderObject uv = IPxlSdrBuf.find_object(psb, "vTexCoord");
    ShaderObject mat_id_pxl = IPxlSdrBuf.new_object(psb, Float4_Obj, "MaterialIDPixel", 1);
    ...
    IPxlSdrBuf.add_branch_node(psb, sn);
    ...
    Tree vtx_param_src_tree = IVtxSdrBuf.sell_param_source_object_tree(vsb);
    Tree pxl_param_src_tree = IPxlSdrBuf.sell_param_source_object_tree(psb);
