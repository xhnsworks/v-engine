...
Pass RenderSystem_register_shadow_pass(VertexDecl dec)
{
    VertexDecl reg_dec = RenderSystem_register_vertex_declaration(dec);
    var key, data;
    key.vptr_var = reg_dec;
    Iterator iter = Tree_find(g_shadow_pass_tree, key, &data);
    if (iter)
        return data.vptr_var;
    else
    {
        key.vptr_var = reg_dec;
        data.vptr_var = create_shadow_pass(reg_dec);
        Tree_insert(g_shadow_pass_tree, key, data);
        return data.vptr_var;
    }
}
...
Pass RenderSystem_register_depth_pass(PassDecl dec)
{
    ...
    else
    {
        ...
        Pass ret = create_std_pass_from_dec(&status);
...
Pass RenderSystem_register_std_pass(PassDecl dec)
{
    ...
    else
    {
        ...
        status.render_pipes[0].out = MaterialIDOutput;
        status.render_pipes[1].out = ColorOutput;
        status.render_pipes[2].out = NormalTangentOutput;
