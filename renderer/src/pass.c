...
void Pass_render(Pass _self, VertexBuffer _vbf, IndexBuffer _ibf, uint32 _num_idxs, mesh_mode _mode)
{
    if (to_ptr(_self))
    {
        if(!_self->linked)
...
void Pass_render_plane(Pass _self, RenderablePlane _pln)
{
    ...
    Pass_render(_self, vbf, ibf, 6, Triangular);
}
...
void Pass_auto_set_uniform_params(Pass _self, Renderer _rdr)
{
        ...
        if (_param_source_tree)
        {
            ...
            while (iter)
            {
                ...
                if (Renderer_is_uniform_param_source(_rdr, src))
                {
                    ...
                    EAssert(array_size > 0, "param array size less equal zero!");
                    ...
                    case Int_Obj:
                        ...
                        break;
                    case Float_Obj:
                        if (array_size == 1) { float* p = shader_object_value.value; Pass_set_uniform_param_float(_self, name, *p); }
                        else                 { float* p = shader_object_value.value; Pass_set_uniform_param_floats(_self, name, p, array_size); }
                        break;
                    case Float2_Obj:
                        if (array_size == 1) {
                        ...
                        break;
                    case Float3_Obj:
                        if (array_size == 1) { EFloat3* p = shader_object_value.value; Pass_set_uniform_param_float3(_self, name, *p); }
                        else                 { EFloat3* p = shader_object_value.value; Pass_set_uniform_param_float3s(_self, name, p, array_size); }
                        break;
                    case Float4_Obj:
                    ...
                    case Texture1D_Obj:
                        ...
                        break;
                    case Matrix4x4_Obj:
                        if (array_size == 1) { Matrix4x4 p = shader_object_value.value; Pass_set_uniform_param_mat4x4(_self, name, p); }
                        else                 { Matrix4x4 p = shader_object_value.value; Pass_set_uniform_param_mat4x4s(_self, name, p, array_size); }
                        break;
                    case Bool_Obj:
                        if (array_size == 1) {
                            ///
