...
IndexBuffer _IndexBuffer_new(mesh_mode mode, const char* _file, uint _line)
{
    index_buffer* buffer = _Malloc(sizeof(index_buffer), _file, _line);
    buffer->face_size = face_size(mode);
    buffer->index_buffer = Malloc(DEFAULT_INDEX_BUFFER_SIZE);
...
void _IndexBuffer_grow_up(IndexBuffer _self, uint num_faces)
{
    ...
    {
        char* tmp = Malloc(totel_idx_buf_size * 2);
...
uint32 IndexBuffer_attach_mesh(IndexBuffer _self, Mesh _mesh)
{
...
mesh_mode IndexBuffer_get_mesh_mode(IndexBuffer _self)
{
