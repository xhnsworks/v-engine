...
MaterialInstance MaterialInstance_new(const char* mat_name, const char* _col_tex_file, const char* _nor_tex_file)
{
    MaterialInstance ret;
    ret = Malloc(sizeof(material_instance));

    Texture2D null_tex = {NULL};
    if (_col_tex_file && strcmp(_col_tex_file, "") != 0)
        ret->col_tex = RenderSystem_register_texture2d(_col_tex_file);
    else
        ret->col_tex = null_tex;
    if (_nor_tex_file && strcmp(_nor_tex_file, "") != 0)
        ret->nor_tex = RenderSystem_register_texture2d(_nor_tex_file);
    else
...
**/
Texture2D MaterialInstance_get_color_texture(MaterialInstance _self)
{
    return _self->col_tex;
}

Texture2D MaterialInstance_get_normal_texture(MaterialInstance _self)
{
...
MaterialInstance MaterialInstance_clone(MaterialInstance _self)
{
    ...
    ret->col_tex = Texture2D_clone(_self->col_tex);
    ret->nor_tex = Texture2D_clone(_self->nor_tex);
    return ret;
}
