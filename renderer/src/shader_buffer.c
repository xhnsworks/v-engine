...
{
    Tree ret = _sb.self->param_src_obj_tree;
    _sb.self->param_src_obj_tree = NULL;
    return ret;
}
...
void ShaderObject_push_immediate_shader_object(ShaderBuffer _sb, ShaderObject _so)
{
    vptr so_ptr = to_ptr(_so);
    if (so_ptr)
    {
        apush(_sb.self->imme_shader_objects, _so);
    }
}
...
void ShaderBuffer_release(struct _shader_buffer* _buf)
{
    ...
    **/
    ...
    _buf->function = EString_new(get_string_buffer);
    ///tmp = mbuf;
    string_buffer_clear;
    ...
    _buf->output = EString_new(get_string_buffer);
}
...
**/
    ...
    Tree_insert(_sb.self->param_src_obj_tree, key, data);
    return ret;
}
...
**/
    ...
    var key, data;
    key.uint32_var = _src;
    EAssert(!Tree_find(_sb.self->param_src_obj_tree, key, &data), "insert uniform fail!");
...
ShaderObject _ShaderBuffer_add_uniform_from_renderer(ShaderBuffer _self, Renderer _rdr, sint32 _id, const char* _unif,
                                                     const char* _file, uint _line)
{
    RendererParamEntry ent = Renderer_get_param_entry(_rdr, _id);
    if (!ent)
    {
        ShaderObject null_obj = {NULL};
        return null_obj;
...
ShaderObject ShaderBuffer_new_object(ShaderBuffer _sb, shader_object_type _type, const char* _name, uint32 _array_size)
{
    ...
    **/
    ...
    if (_array_size <= 1)
    {
        ...
        _sb.self->main = EString_new(get_string_buffer);
    }
    else
    {
...
ShaderObject ShaderBuffer_new_immediate_float_object(ShaderBuffer _sb, float _ft)
{
    ...
    ShaderObject_push_immediate_shader_object(_sb, ret);
    return ret;
}
...
void ShaderBuffer_add_prototype_node(ShaderBuffer _sb, ShaderNode _sn)
{
    if (ShaderNode_get_name(_sn))
    {
        ...
        ShaderNode sn = snb->shader_node_clone_proc(_sn, __FILE__, __LINE__);
        var key;
        var data;
...
ShaderNode _ShaderBuffer_add_reference_node(ShaderBuffer _sb, const char* _name, const char* _file, uint _line)
{
    ...
    if (Tree_find(_sb.self->shader_node_prototype_tree, key, &data))
    {
        ...
        **/
        ///return csn;
        ShaderNode sn = {(struct _shader_node*)data.vptr_var};
        return CircuitBoard_add_reference_node(&_sb.self->circuit_board, sn);
    }
    else
    {
        ShaderNode ret = {NULL};
...
{
    _CircuitBoard_add_reference_node(&_sb.self->circuit_board, _bn, _file, _line);
}
