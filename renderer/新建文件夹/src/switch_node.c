...
void SwitchNode_Dest(SwitchNode _self, const char* _file, uint _line)
{
    ...
    while (iter)
    {
        ...
        sint* tags = key.vptr_var;
        array_delete(tags);
        CircuitBoard board = data.vptr_var;
...
{
    SwitchNode ret = Malloc(sizeof(switch_node));
    ret->base.shader_node_init_proc = SwitchNode_Init;
    ret->base.shader_node_dest_proc = SwitchNode_Dest;
    ret->base.shader_node_clone_proc = SwitchNode_clone;
    ret->base.compile_proc = SwitchNode_compile;
    ...
    while (iter)
    {
        ...
        CircuitBoard src_bd = data.vptr_var;
...
void SwitchNode_Init(SwitchNode _self, const char* _file, uint _line)
{
    _self->base.shader_node_init_proc = SwitchNode_Init;
    _self->base.shader_node_dest_proc = SwitchNode_Dest;
    _self->base.shader_node_clone_proc = SwitchNode_clone;
    _self->base.compile_proc = SwitchNode_compile;
...
{
    ShaderNodeBase_delete(_self, _file, _line);
}
