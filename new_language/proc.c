...
{
    printf(str);
    const char* type_str = _get_type_str(value.type);
...
{
    const char* type_str = _get_type_str(_sym.type);
    printf("TYPE %s, VALUE ", type_str);
    if (_sym.type != Unknown)
...
void AddParam(ParserEnv* e, SymbolValue _sym, SymbolValue valuetype)
{
    ...
    if (valuetype.type != StringValue)
    {
        ...
        sv->data.mem_addr.offs = ValueAllocator_alloc(&e->vari_alloc, valuetype.type);
    }
    else
    {
        sv->data.mem_addr.mem_type = VariStrMem;
        sv->data.mem_addr.offs = ValueAllocator_alloc(&e->vari_str_alloc, valuetype.type);
    }
...
static inline SymbolValue _init_value_assign(ParserEnv* e, SymbolValue dst, SymbolValue src)
{
    ...
    sv->type = src.type;
    sv->data.mem_addr.mem_type = VariMem;
    sv->data.mem_addr.offs = ValueAllocator_alloc(&e->vari_alloc, dst.type);
...
SymbolValue VarDef(ParserEnv* e, SymbolValue sym, SymbolValue var_type)
{
    ...
    if (Unknown == sym.type)
    {
        ...
        sv->data.mem_addr.mem_type = VariMem;
        sv->data.mem_addr.offs = ValueAllocator_alloc(&e->vari_alloc, var_type.type);
...
void MarkEnd(ParserEnv* e)
{
    ...
    /// 填充跳转到出口命令
    SymbolValue _export = _alloc_cons_value(e, IntValue);
    CommandClosures end_cc = CommandClosures_new(JumpCommand);
    CommandClosures_add_arg(end_cc, _export);
    ...
    for (uint i = 0; i < n; i++)
    {
        ...
        if (mark->end_cmd)
        {
            int* p = _get_ptr(e, mark->end_cmd->args[0]);
            *p = block_export;
        }
        else
        {
            mark->end_cmd = end_cc;
            int* p = _get_ptr(e, end_cc->args[0]);
            *p = block_export;
        }
    }
    /// 对分支检测跳转方位进行重定向
    ...
    int addr = curt_tag;
    int* p = _get_ptr(e, last_mark.begin_cmd->args[0]);
...
void MarkForCond(ParserEnv* e, SymbolValue bool_value)
{
    ...
    }
    {
        SymbolValue _export = _alloc_cons_value(e, IntValue);
        CommandClosures cc = CommandClosures_new(JumpCommand);
        CommandClosures_add_arg(cc, _export);
        FunctionClosures_add_command(e->curt_func, cc);
