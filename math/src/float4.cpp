...
float Float4_dot(float4 _f4_0, float4 _f4_1)
{
    ...
    f4_1.ft4 = _f4_1;
  #ifdef DEBUG
    EAssert(f4_0.r_ptr % 16 == 0, "%s", "float4 error");
    EAssert(f4_1.r_ptr % 16 == 0, "%s", "float4 error");

    EAssert(f4_0.ft128[3] == 0.0f, "%s", "float4 error");
    EAssert(f4_1.ft128[3] == 0.0f, "%s", "float4 error");
  #endif
