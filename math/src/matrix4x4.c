...
void Matrix4x4_projection(Matrix4x4 _mat, float _width, float _height, float _near, float _far)
{
    ...
    _mat->m2 = _mm_setr_ps( 0.0,                   0.0,                   -(_far + _near) / (_far - _near),     -1.0 );
...
sfloat4 Matrix4x4_mul_float4(Matrix4x4 _mat, sfloat4 _ft)///__m128& point, Matrix4x4& matrix, __m128& result)
{
#ifdef USE_SSE
