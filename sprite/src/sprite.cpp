...
Mesh SpriteElement::Build(SpriteRenderer* sprite_renderer) const
{
	if (m_visiable) {
		...
		Mesh ret = sprite_renderer->new_widget_mesh(m_left, m_top, m_width, m_height, u0, u1, v0, v1, 
			m_color_u0v0, m_color_u1v0, m_color_u1v1, m_color_u0v0);
...
void SpriteElement::GetFourBorders(SpriteRenderer* renderer, FourBorders& borders)
{
	renderer->get_four_borders(m_left, m_top, m_width, m_height, borders);
}
...
void SpriteNormalLayer::LoadConfig(const pugi::xml_node& from)
{
	...
	for (; iter != end; iter++)
	{
		...
		pugi::xml_attribute visible = node.attribute("visible");

		SpriteElement sprite_ele;
		sprite_ele.SetFilename(xhn::static_string(filename.value()));
		sprite_ele.m_left = left.as_float();
		sprite_ele.m_top = top.as_float();
		sprite_ele.m_width = width.as_float();
		sprite_ele.m_height = height.as_float();
		...
		sprite_ele.m_visiable = visible.as_bool();
...
void SpriteNormalLayer::SaveConfig(pugi::xml_node& to)
{
	...
	for (; iter != m_elementBuffer.end(); iter++)
	{
		...
		ele.append_attribute("left").set_value(sprite_ele.m_left);
		ele.append_attribute("top").set_value(sprite_ele.m_top);
		ele.append_attribute("width").set_value(sprite_ele.m_width);
		ele.append_attribute("height").set_value(sprite_ele.m_height);
		...
		ele.append_attribute("visible").set_value(sprite_ele.m_visiable);
	}
}
...
void SpriteTextLayer::LoadConfig(const pugi::xml_node& from)
{
	...
	for (; iter != end; iter++)
	{
		...
		for (uint i = 0; i < wtext.size(); i++)
		{
			...
			sprite_ele.m_left = x;
			sprite_ele.m_top = y;
			sprite_ele.m_width = (float)handle.GetGlyph()->width;
			sprite_ele.m_height = (float)handle.GetGlyph()->height;
