...
{
    CommandClosures ret = (CommandClosures)SMalloc(sizeof(command_closures));
...
{
    BranchBlock ret = (BranchBlock)SMalloc(sizeof(BranchBlock));
...
{
	CommandClosures c_func_cc = CommandClosures_new(_c_func);
	SymbolValue sv;
	sv.data.func = _self;
	sv.type = Function;
    CommandClosures_add_arg(c_func_cc, sv);
	FunctionClosures_add_command(_self, c_func_cc);
}
...
{
    return (ForLoopBlock)FixedStack_get_top_ptr(_self->for_loop_stack);
}
...
{
	e->lex_state.remainder = 1024;
    e->lex_state.string_size = 0;
    e->lex_state.curtSymType = empty;
    e->lex_state.str = e->lex_state.mbuf;
	e->lex_state.mbuf[0] = 0x00;
}
...
FunctionClosures ParserEnv_find_function(ParserEnv* e, const char* _func_name)
{
    ...
    if (Tree_find(e->func_tree, key, &data))
        return (FunctionClosures)data.vptr_var;
    else
        return NULL;
}
...
{
    SymbolValue* v = SymbolStack_insert_unknown_symbol(&e->sym_stack, str);
